...
class OpenAICompatProvider : public AIProvider {
    Q_OBJECT
public:
    explicit OpenAICompatProvider(const QString &providerId,
                                   const QString &name,
                                   const QString &desc,
                                   bool needsKey,
                                   bool local,
    ...
    int contextLimit() const override { return m_ctxLimit; }
    QList<QString> defaultModels() const override { return m_defaultModels; }
    bool isAvailable() const override;

    void setEndpoint(const QString &baseUrl, const QString &chatPath,
                     const QString &apiKey = QString());
    void setDefaultModels(const QList<QString> &models) { m_defaultModels = models; }
